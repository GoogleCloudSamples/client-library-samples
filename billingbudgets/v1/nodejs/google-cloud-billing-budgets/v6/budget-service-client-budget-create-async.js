...
// [START billingbudgets_v1_budgetservice_budget_create_async]
 ...
 */
async function createBudget(billingAccountId = '000000-000000-000000') {
  ...
  try {
    ...
    console.log(
      `  Budget Amount: ${totalAmount} ${response.amount.specifiedAmount.currencyCode}`,
    );
    console.log(`  Threshold Rules: ${response.thresholdRules.length}`);
  } catch (err) {
    if (err.code === status.ALREADY_EXISTS) {
      console.log(
        `Budget '${displayName}' already exists for billing account '${billingAccountId}'.`,
      );
      console.log(
        'Consider updating the existing budget instead or using a different budget ID.',
      );
    } else if (err.code === status.PERMISSION_DENIED) {
      console.error(
        `Permission denied when creating budget for billing account '${billingAccountId}'.`,
      );
      console.error(
        'Ensure the service account has the "billing.budgets.create" permission on the billing account.',
      );
      console.error(
        'Also, if a Pub/Sub topic is specified, ensure the service account has "pubsub.topics.setIamPolicy" permission on the topic.',
      );
    } else {
...
// [END billingbudgets_v1_budgetservice_budget_create_async]
