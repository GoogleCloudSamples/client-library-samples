...
def list_autoscaling_policies(
    ...
    try:
        ...
        print(
            f"Listing autoscaling policies for project '{project_id}' in region '{location}':"
        )
        ...
        for policy in page_result:
            ...
            if policy.basic_algorithm:
                print(
                    f"    Basic Algorithm Cooldown Period: {policy.basic_algorithm.cooldown_period.seconds}s"
                )
            print(
            ...
            if policy.secondary_worker_config:
                print(
                    f"    Secondary Worker Config: min_instances={policy.secondary_worker_config.min_instances}, max_instances={policy.secondary_worker_config.max_instances}"
                )
            print("\n")
        ...
        print(
            f"Error: The specified project or region was not found.\nDetails: {e}",
            "Please ensure the project ID and region are correct and that you have the necessary permissions.",
            file=sys.stderr,
        )
    except exceptions.InvalidArgument as e:
        print(
            f"Error: Invalid argument provided in the request.\nDetails: {e}",
            f"This might happen if the parent format is incorrect. Ensure it's 'projects/{project_id}/regions/{location}'.",
            file=sys.stderr,
        )
...
if __name__ == "__main__":
    ...
    list_autoscaling_policies(args.project_id, args.location)
