...
async function listInspectTemplates(projectId, location = 'global') {
  ...
  try {
    ...
    if (templates.length === 0) {
      console.log(
        `No inspect templates found for ${projectId} in ${location}.`,
      );
      return;
    }
    ...
    if (err.code === status.NOT_FOUND) {
      console.error(
        `Error: Parent resource for ${projectId} in ${location} not found. Please check the project ID and location.`,
      );
    } else {
