...
# [START retail_v2_controlservice_delete_control]
...
def delete_control(
    ...
    try:
        client.delete_control(name=name)
        print(f"Control {control_id} deleted successfully.")
    except NotFound:
        print(
            f"Control {control_id} not found. It may have already been deleted or never existed."
        )
    except Exception as e:
        print(f"Error deleting control {control_id}: {e}")


# [END retail_v2_controlservice_delete_control]

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Delete a Retail Control.")
    parser.add_argument(
        "--project_id",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The ID of the control to delete (e.g., 'my_control_id').",
    )
