...
# [START modelarmor_v1_update_template]
...
def update_model_armor_template(
    ...
    This function demonstrates how to update a specific field (e.g., display_name)
    of an existing Model Armor template using the `update_template` method.
    It uses a field mask to specify which fields should be updated.
    ...
    update_mask = field_mask_pb2.FieldMask(
        paths=["template_metadata.enforcement_type"]
    )
    ...
    try:
        ...
        print(f"Successfully updated template: {response.name}")
        print(f"Updated at: {response.update_time.isoformat()}")
...
# [END modelarmor_v1_update_template]

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Updates a Model Armor template.")
    parser.add_argument(
        "--project_id",
        required=True,
        type=str,
