...
def get_phrase_set(
    ...
    try:
        ...
        print(f"Error: Phrase set '{phrase_set_name}' not found.", file=sys.stderr)
        print(
            "Please ensure the project ID, location, and phrase set ID are correct.",
            file=sys.stderr,
        )
        print(
            "You might need to create the phrase set first if it does not exist.",
            file=sys.stderr,
        )
