...
def create_phrase_set(project_id: str, phrase_set_id: str, phrases: list[str]) -> None:
    ...
    try:
        ...
        print(
            f"Phrase set '{phrase_set_id}' already exists in '{project_id}'.",
            file=sys.stderr,
        )
        print(
            "Consider updating it instead or using a different phrase_set_id.",
            file=sys.stderr,
        )
    except GoogleAPICallError as e:
        print(f"Error creating phrase set: {e}", file=sys.stderr)
