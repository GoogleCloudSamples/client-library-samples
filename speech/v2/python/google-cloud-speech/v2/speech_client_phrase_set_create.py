...
def create_phrase_set(
    ...
    Args:
        ...
        phrase_set_id: The ID to use for the PhraseSet.
    """
    ...
    try:
        ...
        print(
            f"Error: PhraseSet '{phrase_set_id}' already exists in {parent}.",
            file=sys.stderr,
        )
        print(
            "Please try a different phrase_set_id or delete the existing one. "
            f"Details: {e}",
            file=sys.stderr,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Creates a PhraseSet in Google Cloud Speech-to-Text V2."
    )
    parser.add_argument(
        "--project_id",
        type=str,
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "The ID to use for the PhraseSet. This will be the final "
            "component of the PhraseSet's resource name "
            "(e.g., 'my-unique-phrase-set-123')."
        ),
    )
