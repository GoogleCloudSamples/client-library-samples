...
def get_phrase_set(
    ...
    try:
        ...
        print(f"State: {phrase_set.state.name}")

    except NotFound:
        print(f"Error: PhraseSet '{phrase_set_id}' not found.", file=sys.stderr)
        print("Please ensure the PhraseSet ID is correct.", file=sys.stderr)
        print(
            "You might need to create the PhraseSet first if it does not exist.",
            file=sys.stderr,
        )
    except GoogleAPICallError as e:
        print(f"An API error occurred: {e}", file=sys.stderr)
        print(
            "Please check your project ID, location, and network connection.",
            file=sys.stderr,
        )
