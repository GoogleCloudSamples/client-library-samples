...
def create_folder(
    ...
    try:
        ...
        print(
            f"Please ensure your service account has the necessary permissions (e.g., storage.folders.create)."
        )
        print(f"Details: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bucket_name",
        required=True,
        help="The name of the bucket where the folder will be created. This bucket must have hierarchical namespace enabled.",
    )
    parser.add_argument(
        "--folder_name",
        required=True,
        help="The full name of the folder to create, including all its parent folders. Must end with a slash (e.g., 'my-new-folder/').",
    )
